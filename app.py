...
def patient_profile():
    if request.method == "POST":
        ...
        lifestyle_info = request.form.get("lifestyle_info", "").strip()
        smoking_alcohol_status = request.form.get("smoking_alcohol_status", "").strip()
        role = request.form.get("role", "patient").strip()

        db.save_or_update_patient_profile(
            request.user_id, height, weight, blood_group, allergies, chronic_diseases,
            current_medications, previous_surgeries, family_history, lifestyle_info,
            smoking_alcohol_status, role
        )
        flash("Digital Health Profile updated successfully!", "success")
        return redirect(url_for("patient_profile"))

    profile = db.get_patient_profile(request.user_id)
    return render_template("patient_profile.html", profile=profile)

@app.route("/report-analytics", methods=["GET", "POST"])
@login_required
def report_analytics():
    if request.method == "POST":
        report_name = request.form.get("report_name", "Lab Report").strip()
        report_date = request.form.get("report_date", datetime.now().strftime("%Y-%m-%d"))
        report_text = request.form.get("report_text", "").strip()

        # Run Medical Report Agent
        analysis = multi_agent.report_agent.process(report_text)
        
        db.add_medical_report(
            request.user_id, report_name, "Lab Test", report_date,
            analysis["parsed_metrics"], file_path=""
        )
        db.add_health_timeline_event(
            request.user_id, "Report Upload", f"Uploaded Report: {report_name}",
            f"Parsed metrics: {json.dumps(analysis['parsed_metrics'])}", event_date=report_date
        )
        flash("Medical Report processed and correlated successfully!", "success")
        return redirect(url_for("report_analytics"))

    reports = db.get_user_medical_reports(request.user_id)
    return render_template("report_analytics.html", reports=reports)

@app.route("/health-timeline")
@login_required
def health_timeline():
    timeline_events = db.get_user_health_timeline(request.user_id)
    return render_template("timeline.html", events=timeline_events)
...
def reminders():
    if request.method == "POST":
        reminder_type = request.form.get("reminder_type", "Health Checkup")
        title = request.form.get("title", "").strip()
        schedule_time = request.form.get("schedule_time", "")
        if title and schedule_time:
            db.add_scheduled_reminder(request.user_id, reminder_type, title, schedule_time)
            flash("Smart follow-up reminder scheduled!", "success")
            return redirect(url_for("reminders"))

    reminders_list = db.get_user_reminders(request.user_id)
    return render_template("reminders.html", reminders=reminders_list)

@app.route("/analytics")
@login_required
def analytics_dashboard():
    assessments = db.get_assessments_by_user(request.user_id)
    reports = db.get_user_medical_reports(request.user_id)
    profile = db.get_patient_profile(request.user_id)
    stats = db.get_dashboard_stats(request.user_id)
    ...
    if len(assessments) > 5:
        health_score += 5
    health_score = max(30, min(99, health_score))
...
@app.route("/explainable-ai/<int:assessment_id>")
@login_required
def explainable_ai(assessment_id):
    assessment = db.get_assessment_by_id(assessment_id, request.user_id)
    if not assessment:
    ...
    profile = db.get_patient_profile(request.user_id)
    triage_result = multi_agent.run_full_triage(
    ...
    return render_template("explainable_ai.html", assessment=assessment, triage=triage_result)
...
def doctor_dashboard():
    ...
    if profile.get("role") not in ["doctor", "admin"]:
        flash("Access Restricted: Doctor role required.", "warning")
        return redirect(url_for("dashboard"))

    all_recent = db.get_assessments_by_user(request.user_id)
    return render_template("role_dashboards.html", role="doctor", assessments=all_recent)
...
def admin_dashboard():
    ...
    if profile.get("role") != "admin":
        flash("Access Restricted: Admin role required.", "warning")
        return redirect(url_for("dashboard"))

    stats = db.get_dashboard_stats(request.user_id)
    return render_template("role_dashboards.html", role="admin", stats=stats)
