...
def dashboard():
    if request.method == "POST":
        ...
        assessment_id = db.create_assessment(
            ...
            current_medications=current_medications or None,
            pregnancy_status=pregnancy_status or None,
            confidence_scores=json.dumps(assessment.get("confidence_scores", [])),
            medical_references=json.dumps(assessment.get("medical_references", [])),
            red_flag_detected=assessment.get("red_flag_detected", 0),
            temperature=temperature or None,
            temperature_unit=temperature_unit or None
        )
...
def assessment_chat(assessment_id):
    ...
    reply = generate_chat_reply(user_message, assessment, chat_messages, lang)
